...
class ActivityDataExtract():
    ...
    def csv_to_json_convertor(self,file_path):
        try:
            ...
            records=list(json.loads(data.T.to_json()).values()) #Transpose 
            return records
        except Exception as e:
            raise ActivityException(e,sys)
...
if __name__=='__main__':
    FILE_PATH="D:\ActivityP\Activity_Data\ActivityData.csv"
    DATABASE="SDB"
    Collection="ActivityData"
    activityobj=ActivityDataExtract()
    records=activityobj.csv_to_json_convertor(file_path=FILE_PATH)
    print(records)
